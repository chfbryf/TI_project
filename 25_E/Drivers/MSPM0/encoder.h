...
extern volatile int32_t counter_R_A;   /* 右轮编码器 A 相脉冲计数（带方向） */
extern volatile int32_t counter_L_A;   /* 左轮编码器 A 相脉冲计数（带方向） */

/* ---------- API ---------- */
void Encoder_Init(void);            /* 编码器初始化（GPIO已由SysConfig配置） */
void Encodering(void);              /* GPIO 中断中调用：脉冲累加 + 方向判断  */
void Encoder_UpdateSpeeds(void);    /* 50ms 定时器 ISR 中调用：计算速度并清零计数器 */
float GetSpeed_L(void);             /* 获取左轮当前速度，m/s                    */
float GetSpeed_R(void);             /* 获取右轮当前速度，m/s                    */
